...
<p align="center">
  <img src="resources/股票系统架构.png" width="640" alt="股票系统架构">
</p>

* 现实世界数据：主要是使用爬虫抓取各大新闻网站、期货市场、热点程序的数据。  
  目录 `crawler` 是一个完整的爬虫系统，从各大网站中爬取信息，经过本地AI进行加工，总结成研报，并且可以通过API自动发送到手机端。
* 券商数据库：支持不同的券商数据，利用券商统一接口将数据和系统隔离。  
  目录 `gateways` 集成各大券商的数据库，这里没有去抓取第三方的免费数据，需要找券商开通响应的权限接入。
* 人工智能模块：将现实世界数据和券商数据进行整合，作出对于未来的预测。  
* 常规操作：支持全市场根据一定的策略选股，查看个股K线图，计算常用数据。  
  目录 `tools` 包括很多常规的工具，查看满足条件的股票。
* 股票预测：需要预测的内容有很多，比如指数的涨跌，K线走势，分析行业走势等详细信息。

本项目集成了 MCP（Model Context Protocol） 交互引擎，支持通过自然语言对话（如输入“苹果股票”）直接智能识别标的与操作意图，自动联动更新界面参数并渲染最新行情图表，让复杂的量化分析与数据查看变得像聊天一样简单高效。

<p align="center">
  <img src="resources/量化界面.png" width="640" alt="量化界面">
  <br>
...
运行本数据库有几个难点，一个是券商数据，一个是AI训练。为了方便不同类型的人需求，将系统拆分成不同的独立模块，尽量每个模块既能够独立运行，又能够组装成一个完整的系统。想要快速了解本仓库，可以查看 [quick_start.py](quick_start.py) 的代码，它是一个基本的 python 程序，已经准备好数据，同时不需要 AI 就能运行。

本项目采用模块化设计，将数据承载、逻辑筛选与结果展示分离，确保代码的高可维护性：

* **crawler**

`crawler` 目录负责多平台内容采集与数据获取，支持从不同来源自动抓取信息，为后续 AI 分析、内容生成以及自动发布到各大内容平台提供数据基础。
...
- [ ] 外围联动分析： 利用交易时差，参考美股、期货等外围市场的波动，预判并指导A股走势。
