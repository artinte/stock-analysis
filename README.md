...
这是一个面向股票投资研究的数据与分析平台，主要整合行情数据、财经资讯、AI 分析、股票交易和可视化展示等功能。
...
不同券商和数据提供方采用独立的 Gateway 实现。

新增一个数据源时，只需要实现对应的 Gateway，而不需要修改上层业务逻辑，从而实现数据源的方便接入和切换。

### Task 化设计

数据爬取采用 Task 化设计。

每个网站或数据类型对应独立任务，可以单独执行，也可以根据实际需求组合多个任务，方便扩展和维护。
