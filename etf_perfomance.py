...
def analyze_and_plot(file_path):
    ...
    try:
        ...
        # 模糊匹配列名
        name_col = next((col for col in df.columns if '名称' in col and '成分' in col), None)
        if not name_col:
        ...
        for bar in bars:
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, height + 0.1, 
                    f'{height:.2f}%', va='bottom', ha='center', fontsize=9, rotation=90)

        # 装饰图表
        ax.set_title(f'中证 931380 指数 - 全量成分股权重分布 (共 {num_items} 只)', fontsize=16, pad=25)
