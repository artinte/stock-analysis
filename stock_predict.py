...
class EnhancedTransformer(nn.Module):
    def __init__(self, feature_dim=8, model_dim=256, nhead=8, num_layers=3, pred_len=5):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=model_dim, nhead=nhead, dim_feedforward=256, 
...
class StockDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        future_closes = self.data[idx + self.seq_len : idx + self.seq_len + self.pred_len, 3]
...
if dm.start(config):
    try:
        # 1. 获取 10 年数据
        klines = dm.get_kline("600519.SH", Interval.DAY_1, 
                              datetime.now() - timedelta(days=365*20), datetime.now())
        
        # 2. 特征工程 & 查杀 NaN
        df = pd.DataFrame([{'o':k.open,'h':k.high,'l':k.low,'c':k.close,'v':k.volume} for k in klines])
        ...
        model.eval()
        with torch.no_grad():
            last_window = torch.FloatTensor(scaled_data[-SEQ_LEN:]).unsqueeze(0).to(device)
