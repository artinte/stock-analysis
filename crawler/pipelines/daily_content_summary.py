...
def generate_daily_content_summary(
    ...
    system_prompt = (
        ...
        "[CONTENT]\n"
        "一、今日市场核心结论\n"
        "这里直接开始正文，绝对不要再次写文章标题。\n"
        "二、今日最值得关注的事件\n"
        "这里继续正文。\n"
        "[/CONTENT]\n\n"
        "【非常重要】\n"
        "你必须把[TITLE]和[CONTENT]视为两个完全独立的区域。\n"
        "[TITLE]中的标题只输出一次。\n"
        "[CONTENT]中绝对不要再次输出[TITLE]中的标题。\n"
        "[CONTENT]的第一行必须直接是“一、今日市场核心结论”。\n"
        "[CONTENT]第一行禁止使用#、##、###生成文章标题。\n"
        "[CONTENT]中不要添加任何文章总标题，只允许按照下面规定的正文结构进行内容组织。\n"
        "不要在[TITLE]、[CONTENT]之外输出任何内容。\n\n"
        "【正文结构】\n"
        "一、今日市场核心结论\n"
        ...
        "输出前自行检查：\n"
        "1. [TITLE]中是否只有标题。\n"
        "2. [CONTENT]是否直接从“一、今日市场核心结论”开始。\n"
        "3. [CONTENT]开头是否错误地重复了标题。\n"
        "4. 标题是否出现了#、##、###等Markdown符号。\n"
        "5. 是否严格只输出[TITLE]和[CONTENT]两个区域。\n"
    )
    user_prompt = (
        "请根据以下已经提炼好的财经新闻摘要，"
        "生成一份《每日股票重点关注信息》。\n\n"
