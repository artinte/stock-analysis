...
def print_fetched_articles(articles: List[Any], max_content_len: int = 500) -> None:
    """
    ...
    for idx, item in enumerate(articles, 1):
        ...
        time_str = fetched_at.strftime("%Y-%m-%d %H:%M:%S") if hasattr(fetched_at, "strftime") else str(fetched_at)

        # 确定主内容显示：优先显示正文，若正文为空则显示摘要
        main_text = content.strip() if content else (summary.strip() if summary else "（无正文及摘要）")
...
def save_raw_articles_to_txt(articles: List[Any], output_file: str = "output/raw_fetched_articles.txt") -> None:
    """
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"==================== 📊 共抓取到 {len(articles)} 条原生内容 ====================\n\n")
        for idx, item in enumerate(articles, 1):
            if isinstance(item, dict):
                source = item.get("source_name", "未知来源")
                category = item.get("category", "")
