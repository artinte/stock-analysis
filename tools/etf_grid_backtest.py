...
买入间距：0.56%（下跌触发）。
卖出间距：0.59%（上涨触发）。
实战频率：目标日均成交 1 次（1 买 1 卖）。

动态策略最忌讳的是单边暴跌或者单边暴涨，如果处于这种行情，会非常容易卖飞。因为单边暴跌会让你过快打满 15 万的上限，单边暴涨则会让你过早触及 1.5 万的下限。

优化后的 2.0 版：网格参数设定

1. 执行策略：不对称增强型网格 (Asymmetric Accumulation Grid)

执行标的： 中证 A500 ETF (159361)。理由：流动性极佳，买卖价差（Spread）极小，适合 0.5% 级别的精细操作。

买入单位 (Buy Unit)： 4,200 股。
卖出单位 (Sell Unit)： 4,000 股。

逻辑优化： 每一轮“买入+卖出”闭环，账户自动沉淀 200 股。按每日 1 次轮回计算，精准实现每日净增 200 股的目标。
...
逻辑优化： 买入阈值略小于卖出阈值（0.56% < 0.59%）。在震荡市中，程序会更敏感地捕捉下跌买入机会，确保你的“定投储备”能优先成交。
注意：因为股价的最小单位是分 0.001，因此触发买入和卖出的价格点可能会有微小的偏差，但整体逻辑保持不变。

3. 绩效目标与财务预算

单次轮回利润： 约 28.3 元（1.2 元股价 × 4000 股 × 0.59%）。

每日定投成本：
200 股总市值：约 240 元。
扣除 2 次轮回利润 (56.6 元)：每日实际支出约 183.4 元，每年假设 240 次交易，投入金额 44,016 元。

注：如果你想严格控制在 100 元，可改为每天只做 1 次“买 4200 / 卖 4000” 股。

最终逻辑：

A500 现价 1.320 元，底仓 60000 股
每涨 0.59% 卖出 4000 股
每跌 0.56% 买入 4200 股
买卖频次：365 * 2 + 10 次
因为价格会上涨，卖的次数会稍微多余买。
预计未来一年 A500 上涨 10% ，预测未来一年的利润。


三、 收益预测与逻辑对冲

1. 波动增厚 vs 银行利息
单日目标：1 次买卖成功，产生利润约 32 元。
年化预估：32 元 × 360 交易日 ≈ 11,520 元。
对冲价值：即便指数价格一年内一分钱不涨，这 10% 的额外收益也足以秒杀 2.5% 的银行定期利息，并有效覆盖指数回撤带来的心理压力。
